<template>
  ...
  totalClass: 'total',
            ...
            <SwiperSlide v-for="(slide, index) in about[0].slider" :key="index">
          ...
          @click.prevent
          ><span>Скачать</span></the-button
        >
          </div>
        </div>
        <div class="description" v-html="$replaceNewLines(about[0].description)">
        </div>
      </div>
    </div>
  </div>
...
import TheAboutListVue from "./../About/TheAboutList.vue";
